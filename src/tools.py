...
def _query_ticker_data(ticker: str, columns: str, limit: int) -> tuple[list[dict], str | None]:
    try:
        df = db_manager.client.query_df(
            f"SELECT {columns} FROM prices WHERE upper(symbol) = upper(%(symbol)s) ORDER BY timestamp DESC LIMIT %(limit)s",
        ...
        logger.error(f"Error in ticker query ({columns}): {e}")
        return [], str(e)
...
def tool_get_ticker_prices(ticker: str, limit: int = DEFAULT_PRICES_LIMIT) -> tuple[list[dict], str | None]:
    ...
    return _query_ticker_data(ticker, "timestamp, symbol, price, volume", limit)
...
def tool_get_ticker_indicators(ticker: str, limit: int = DEFAULT_PRICES_LIMIT) -> tuple[list[dict], str | None]:
    ...
    return _query_ticker_data(ticker, "timestamp, symbol, price, volume, SMA_5, SMA_20, RSI, MACD, MACD_Signal", limit)
...
def _setup_price_axis(ax: plt.Axes, df: pd.DataFrame, title: str) -> None:
...
def _setup_rsi_axis(ax: plt.Axes, df: pd.DataFrame) -> None:
    ax.plot(df['timestamp'], df['RSI'], color='#9467bd', linewidth=1.8, label='RSI (14)')
    ax.axhline(70, color='#d62728', linestyle='--', alpha=0.7, label='Overbought (70)')
    ax.axhline(30, color='#2ca02c', linestyle='--', alpha=0.7, label='Oversold (30)')
    ax.fill_between(df['timestamp'], 30, 70, color='#9467bd', alpha=0.1)
...
def tool_generate_market_chart(
    ...
    try:
        data_records, err = tool_get_ticker_indicators(ticker=ticker, limit=limit)
        if err or not data_records:
            return f"Failed to retrieve data for {ticker}: {err or 'Empty recordset'}"
        ...
        if chart_type == "rsi":
            ...
            _setup_price_axis(ax_price, df, f"Technical Analysis (Price & RSI) - {ticker}")
            if 'RSI' in df.columns and not df['RSI'].isna().all():
                _setup_rsi_axis(ax_rsi, df)
            ...
            _setup_price_axis(ax_price, df, f"Technical Analysis (Price & MACD Momentum) - {ticker}")
            ...
            _setup_price_axis(ax_price, df, f"Price & Trading Volume - {ticker}")
            ...
            _setup_price_axis(ax, df, f"Moving Average Trend (SMA) - {ticker}")
            ...
            _setup_price_axis(ax_price, df, f"Comprehensive Market Analysis - {ticker}")
            if 'SMA_5' in df.columns and not df['SMA_5'].isna().all():
